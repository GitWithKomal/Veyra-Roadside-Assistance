  ...
  useEffect(() => {
    ...
    const fetchNearbyMechanics = async () => {
      try {
        ...
        if (isMounted) {
          console.log("Nearby mechanics updated:", data.mechanics);
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {mechanics.map((mechanic) => (
            ...
            >
              ...
              {/* RATING */}
              <div className="mt-4 flex items-center gap-3 text-xs font-semibold text-[var(--veyra-text-secondary)]">
                <span>⭐ {mechanic.rating || 0}</span>

                <span>•</span>

                <span>
                  {mechanic.servicesOffered?.length || 0} services
                </span>
              </div>

              {/* SERVICES PREVIEW */}
              <div className="mt-4 flex flex-wrap gap-2">
                {(mechanic.servicesOffered || []).slice(0, 3).map((service) => (
                  <span
                    key={service._id}
